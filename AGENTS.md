...
- Learning sync is folder-based and provider-neutral: `retype/services/sync.py` owns replica validation/merge/recovery and `docs/source/learning-sync.rst` defines the user-facing scope; never treat legacy `save.json`, `chord-mastery.json`, or `config.json` as the sync protocol.
...
- The Windows test workflow is `.github/workflows/run-tests.yml`; it uses Python 3.11 and the locked `uv` test group. Keep the Windows-only PyQt5-Qt5 5.15.2 pin: later Qt5 wheels omit Windows.
