...
- The one-shot CharaChorder startup contract and supported profile are documented in `docs/source/device-startup.rst`; `retype/services/device_snapshot.py` owns the read and atomic handoff to `BookView.setChords`. The owner document also records the no-per-command-flush invariant and the only safe CML recovery: skip an exact-index malformed payload, but abort on timeout, command, or index errors.
