...
counts are stored as ``chord-mastery.json`` in the active data folder (see
:ref:`user-dir`), so they survive application sessions. Manual
mastered/unmastered overrides are stored there too and win over the measured
progress until they are restored.
